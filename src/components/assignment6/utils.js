...
function groupByCity(data) {
    ...
    let result = data.reduce((result, d) => {
        // Update destination city information
        let currentDestCity = result[d.DestCity] || {
            ...
            "Country": d.DestCountry,
            "Latitude": +d.DestLatitude,
            "Longtitude": +d.DestLongitude
        };
        currentDestCity.Count += 1; // Increment the route count for the destination city
        result[d.DestCity] = currentDestCity;

        // Update source city information
        let currentSourceCity = result[d.SourceCity] || {
            ...
            "Country": d.SourceCountry,
            "Latitude": +d.SourceLatitude,
            "Longtitude":+d.SourceLongitude
        };
        currentSourceCity.Count += 1; // Increment the route count for the source city
        result[d.SourceCity] = currentSourceCity;

        return result;
    }, {});

    // Convert the result object to a sorted list
    result = Object.keys(result).map(key => result[key]);
    result = result.sort((a, b) => b.Count - a.Count); // Sort by route count in descending order
